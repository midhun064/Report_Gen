...
import { X, CheckCircle, XCircle, Clock, AlertCircle, Building, Settings } from 'lucide-react';
import { FormStateNotification } from '../../services/formStateService';

interface MeetingRoomNotificationProps {
  notifications: FormStateNotification[];
  onClose: () => void;
  onMarkRead: (notificationId: string) => void;
}

const MeetingRoomNotification: React.FC<MeetingRoomNotificationProps> = ({
  notifications,
  onClose,
  onMarkRead
}) => {
  ...
  useEffect(() => {
    if (notifications.length > 0) {
      console.log('🔔 Meeting room notifications received:', notifications);
        ...
        <div className="p-4 border-t bg-gray-50">
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">
              {notifications.length} meeting room update{notifications.length !== 1 ? 's' : ''}
            </span>
            <button
              onClick={handleClose}
...
export default MeetingRoomNotification;
